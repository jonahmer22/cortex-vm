;; Sieve of Eratosthenes up to 1000, run 500 times. Expected count: 168.
;; Matches benchmarks/asm/realworld_sieve.s
;; flag[i] is stored as a single byte at linear-memory address i.
(module
  (memory 1)
  (func (export "main") (result i32)
    (local $i i32) (local $p i32) (local $j i32) (local $count i32) (local $outer i32)

    (local.set $outer (i32.const 0))
    (block $outer_exit
      (loop $outer_loop
        (br_if $outer_exit (i32.ge_s (local.get $outer) (i32.const 500)))
